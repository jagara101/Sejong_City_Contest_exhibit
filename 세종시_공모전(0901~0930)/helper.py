...
def ml_ols(data, xnames, yname, degree=1, test_size=0.25, use_scalling=False, random_state=777):
    ...
    result.table=get_ols_table(x_train,xnames,yname,
                               result.intercept,result.coef,result.train_pred)
...
def tf_logit_result(model,fit,x,y):
    ...
    result_df = DataFrame({'정확도(Accuracy)':[acc],'정밀도(Precision)':[pre], '재현율(Recall, TPR)':[recall],
                            '위양성율(Fallout, FPR)': [fallout],'특이성(Specificity, TNR)':[spe], 'RAS': [ras],
                            'f1_score':[f1]})
